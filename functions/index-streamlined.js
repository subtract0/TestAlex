  ...
  try {
    ...
    if (userDoc.exists && userDoc.data().threadId) {
      ...
      try {
        ...
        logger.warn("Existing thread not found, creating new one", {userId, oldThreadId: existingThreadId});
      }
    }
    ...
    logger.error("Error creating thread", {userId, error: error.message});
    throw error;
  }
}
...
exports.clearThread = onCall(async (request) => {
  try {
    ...
    // Update user document with new thread ID
    await admin.firestore().collection("users").doc(userId).set({
    ...
    // Clear user's messages (optional)
    try {
      ...
      logger.warn("Message cleanup failed", {error: cleanupError.message});
    }
